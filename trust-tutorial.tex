...
\frame{\frametitle{What The Appraiser Knows}
  ...
  \begin{itemize}
  \item On initial measurement after startup
    \begin{itemize}
    \item $\extend{0x00..0}{\hash{\mathtt{SINIT Policy}}}$ \texttt{SENTER} and \texttt{SINIT} ran
    \item $\extend{0x00..0}{\hash{\mathtt{MLE}}}$ \texttt{SINIT} measured the MLE
    \item MLE was correct on startup
    \end{itemize}
  \item On measurement after application startup
    \begin{itemize}
    \item $\extend{0x00..0}{\hash{\mathtt{OS}}}$ OS was correct on startup and was started by MLE
    \item $\extend{0x00..0}{\hash{\mathtt{app}}}$ Application was correct on startup and was started by OS
    \end{itemize}
  \end{itemize}

  \begin{block}{Trusted $\neq$ Secure}
    \begin{itemize}
    \item The Appraiser knows what it is talking to
    \item Nothing is protected
    \end{itemize}
  \end{block}
